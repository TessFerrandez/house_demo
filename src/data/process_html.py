...
def extract_address(house):
    ...
    try:
        return (
            house.find(class_="sold-property-listing__heading")
            .find(class_="item-link")
            .get_text()
        )
    except AttributeError:
        return ""
...
def extract_region(house):
    try:
        region = (
            house.find(class_="sold-property-listing__location")
...
def extract_area(house):
    try:
        size_rows = (
            house.find(class_="sold-property-listing__size")
            .find(class_="sold-property-listing__subheading")
            .get_text()
            .splitlines()
        )
...
def extract_sup_area(house):
    try:
        sup_area_str = house.find(
            class_="sold-property-listing__supplemental-area"
        ).get_text()
...
def extract_monthly_fee(house):
    try:
        monthly_fee_str = (
            house.find(class_="sold-property-listing__fee").get_text().strip()
        )
        monthly_fee_str = (
            re.sub(r"\s+", "", monthly_fee_str, flags=re.UNICODE)
            .replace("kr/mån", "")
            .replace("Â", "")
            .replace(",", ".")
        )
...
def convert_to_date(swedish_date):
    us_date = (
        swedish_date.replace("januari", "Jan")
...
def extract_date_sold(house):
    try:
        date_sold = (
            house.find(class_="sold-property-listing__sold-date")
            .get_text()
            .replace("Såld", "")
            .strip()
        )
...
def extract_price_change(house):
    try:
        price_change_pct_str = house.find(
            class_="sold-property-listing__price-change"
        ).get_text()
...
def extract_price(house):
    try:
        price_str = (
            house.find(class_="sold-property-listing__price")
            .find(class_="sold-property-listing__subheading")
            .get_text()
        )
        price_str = (
            clean_numeric_string(price_str).replace("Slutpris", "").replace("kr", "")
        )
...
def process_file(filename: str):
    ...
    houses = [
        {
            "house_type": extract_house_type(house),
